...
interface GamepadMeta extends GamepadProfile {
...
export function attachInputCapture(
  ...
  const ensureGamepadMeta = (gamepad: Gamepad) => {
    ...
    const meta: GamepadMeta = {
      ...createGamepadProfile(gamepad),
      connected: false,
      needsResync: true,
    };
    gamepadMeta.set(gamepad.index, meta);
    // The layout is calibrated once, from the pad at rest. A guest reporting a
    // dead d-pad or trigger is diagnosed from this line and nothing else.
    console.info(`[webrtc] gamepad ${gamepad.index}: ${describeGamepad(meta)}`);
    return meta;
  };
  ...
  const pollGamepads = () => {
    ...
    for (const [padIndex, pad] of pads.entries()) {
      ...
      const snapshot = readGamepadState(pad, meta);
