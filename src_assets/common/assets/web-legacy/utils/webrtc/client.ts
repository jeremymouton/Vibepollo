...
export class WebRtcClient {
  ...
  private async connectAttempt(
    ...
    this.inputChannel.onclose = () => {
      this.stopPing();
      callbacks.onInputChannelState?.('closed');
    };
  ...
  /// Liveness heartbeat. The lwrtc wrapper on the host exposes no peer-connection
  /// state callback, so a tab that closes abruptly leaves its session encoding for a
  /// dead peer forever. The host reaps sessions whose data channel goes silent; this
  /// keeps the channel audibly alive while the page is open. Sent directly (never
  /// queued) — a stale buffered ping delivered after reconnect would be meaningless.
  private startPing(): void {
    this.stopPing();
    this.pingTimer = window.setInterval(() => {
      if (!this.inputChannel || this.inputChannel.readyState !== 'open') return;
      try {
        this.inputChannel.send(JSON.stringify({ type: 'ping' }));
      } catch {
        /* ignore */
      }
    }, 5000);
