...
namespace confighttp {
  ...
  /**
   * @brief Complete a native Moonlight pairing through an invite.
   *
   * Ordinarily the owner types the Moonlight PIN into the admin UI and the new
   * client lands on PERM::_default. This lets the guest finish it themselves,
   * and the resulting client carries the invite's permission instead.
   *
   * Two secrets, proving two different things. The invite PIN proves they were
   * invited at all; the Moonlight PIN proves they are the client currently
   * standing at the door, since it is generated by their own Moonlight and
   * never travels through the link. Holding the invite alone does not let
   * someone bind a permission to a stranger's pairing attempt.
   *
   * Unauthenticated by design.
   * @api_examples{/api/join/@c token/pair| POST| {"pin":"123456","moonlight_pin":"1234","name":"Laptop"}}
   */
  void pairInvite(resp_https_t response, req_https_t request) {
    if (request->path_match.size() < 2) {
      join_api::refuse(response, invite::policy::result_e::unknown, 0);
      return;
    }
    ...
    if (!input->contains("moonlight_pin") || !(*input)["moonlight_pin"].is_string()) {
      bad_request(response, request, "moonlight_pin is required");
      return;
    }

    const auto token = std::string(request->path_match[1]);
    const auto outcome = invite::redeem(token, invite::mode_e::pairing, (*input)["pin"].get<std::string>());

    if (outcome.result != invite::policy::result_e::ok) {
      BOOST_LOG(info) << "Invites: refused a pairing redemption ("sv
                      << static_cast<int>(outcome.result) << ") from "sv
                      << request->remote_endpoint().address().to_string();
      join_api::refuse(response, outcome.result, outcome.retry_after_seconds);
      return;
    ...
    // Name the device after the invite unless the guest offered one, so the
    // owner's client list reads "Brother" rather than "Moonlight Client".
    std::string name = input->value("name", std::string {});
    if (name.empty()) {
      name = outcome.invite.label;
    }
    ...
    if (!paired) {
      ...
      body["error"] = "That PIN did not match a waiting Moonlight client. Start pairing in Moonlight, then try again.";
      SimpleWeb::CaseInsensitiveMultimap headers;
      headers.emplace("Content-Type", "application/json");
    ...
    BOOST_LOG(info) << "Invites: '"sv << outcome.invite.label << "' paired a Moonlight client from "sv
                    << request->remote_endpoint().address().to_string();
  ...
  void start() {
    ...
    register_api_route("^/api/join/([^/]+)/pair$", "POST", pairInvite);
